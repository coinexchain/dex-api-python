...
def wallet_test():
    ...
    # export file
    export_key = lib.ExportKey
    export_key.restype = ctypes.c_char_p
    export_key_bytes = export_key(key_name, password, password)
    export_key_str = str(export_key_bytes, encoding='utf-8')
    key_file = open('../../tmp/cat.txt', 'w+')
    key_file.write(export_key_str)
    key_file.close()
